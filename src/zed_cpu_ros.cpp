...
public:
  ...
  StereoCamera(int device, int resolution, double frame_rate)
    ...
    // // this function doesn't work very well in current Opencv 2.4, so, just
    // use ROS to control frame rate. setFrameRate(frame_rate);

    std::cout << "Stereo Camera Set Resolution: " << camera_->get(WIDTH_ID)
              << "x" << camera_->get(HEIGHT_ID) << std::endl;
    // std::cout << "Stereo Camera Set Frame Rate: " << camera_->get(FPS_ID) <<
    // std::endl;
  }
...
public:
  ...
  ZedCameraROS() {
    ...
    private_nh.param("config_file_location", config_file_location_,
                     std::string("~/SN1267.conf"));
    private_nh.param("left_frame_id", left_frame_id_,
                     std::string("left_camera"));
    private_nh.param("right_frame_id", right_frame_id_,
                     std::string("right_camera"));
    ...
    image_transport::Publisher left_image_pub =
        it.advertise("left/image_raw", 1);
    image_transport::Publisher right_image_pub =
        it.advertise("right/image_raw", 1);

    ros::Publisher left_cam_info_pub =
        nh.advertise<sensor_msgs::CameraInfo>("left/camera_info", 1);
    ros::Publisher right_cam_info_pub =
        nh.advertise<sensor_msgs::CameraInfo>("right/camera_info", 1);
    ...
    if (load_zed_config_) {
      ...
      try {
        getZedCameraInfo(config_file_location_, resolution_, left_info,
                         right_info);
    ...
    while (nh.ok()) {
      ...
      r.sleep();
      // since the frame rate was set inside the camera, no need to do a ros
      // sleep
    }
  }
   ...
   */
  void getZedCameraInfo(std::string config_file, int resolution,
    ...
    double baseline = 0.0;
    // some config files have "Baseline" instead of "BaseLine", check
    // accordingly...
  ...
  void publishCamInfo(const ros::Publisher &pub_cam_info,
                      sensor_msgs::CameraInfo &cam_info_msg, ros::Time now) {
   ...
   */
  void publishImage(cv::Mat img, image_transport::Publisher &img_pub,
                    std::string img_frame_id, ros::Time t) {
